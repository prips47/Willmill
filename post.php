...
<html>
   <?php    include 'head.php'; ?>
    ...
    <?php
         ...
         $select_query = "SELECT user_id,texts,type,type2,image_name FROM newsfeed.post where id=$post_id";
        ...
        $row= mysqli_fetch_array($select_query_result);
          ...
          if($row['type']=="original")
         {
            ...
            ?>
                ...
                <div class="row" style="border:1px solid #1ABC9C; width:550px; margin-left:10px; margin-top:15px" >
                  <div class="row">
                    ...
                    <div class="row rowstyle"style="margin-top:10px;">
                      ...
                      <!--  <div style="float: right;margin-right: 18px" id="comment_count1" > <?php if($comments>0)
                                echo "$comments"." Comments" ?> 
                        </div>-->
                  </div>
                    <div class="row" style="border-top:1px solid #1ABC9C;width: 548px;margin-left:0px;;background-color:#E9F7EF">
                       <div style=" float: left"><div style="margin-top:5px;margin-bottom:5px;margin-left:5px">
...
window.onclick = function(event) {
    if (event.target == modal) {
        modal.style.display = "none";
                ...
                <div class="row rowstyle" style="margin-top:15px;">
                 <div id="comment_box" class="row" style="border:1px solid #1ABC9C; width:548px;margin-left: 25px;" >
                     <textarea style="width:547px;height:60px;" id="comment"></textarea></div>
                     <div style=margin-top:5px;">
                         <button class="btn btn-success btn-sm" style="float:right;margin-right: 125px;" onclick="submitComment(<?php echo $post_id?>,<?php echo $profile_id ?>,<?php echo $comments?>)">Comment</button>
                         <div style="margin-left:30px;" onclick="comment_list(<?php echo $post_id ?> )"><a>previous comments</a></div>
                     </div>
                </div>
                
                <div class="row rowstyle" style="margin-top:15px;" id="comment_list">
       ...
       elseif ($row['type']=="shared") {
            ...
            $image_name=$row['image_name'];
            ?>
              <div class="row" style="border:1px solid #1ABC9C; width:550px; margin-left: 10px; margin-top:25px">
             <div class="row">
                  ...
                  <div class="row" id ="post_content<?php echo $post_id; ?>" onblur="edit(<?php echo $post_id?>);" style="margin-left:15px; margin-top:5px;margin-top: 5px;margin-bottom: 5px" contenteditable="true"><?php echo "$feed"; ?>
                  </div>
                    <div class="row" style="max-width:548px; margin: auto" >
                                   <?php
                                       if ($row['type2']=="image"){
...
<script>
function submitComment(post_id,profile_id,comments){
    var comment=document.getElementById("comment").value;
    $.ajax({
            url:'submitComment.php',
            type:'POST',
            data:{data1:post_id,data2:profile_id,data3:comment},
            datatype:"json",
            success:function(data){
                //alert(data);
                document.getElementById("comment").value="";
                 var x=  document.getElementById("comment_list").innerHTML;
                 var y='<div id="comments" class="row" style="border:1px solid #1ABC9C; width:548px;margin-left: 25px;" ><div style=" margin-top: 10px;padding-bottom: 8px;"><div style="float:left;margin-left: 10px;"><b>'+"<?php echo $user_name ?>"+'</b></div><div style="float:right; margin-top: 5px;margin-right:10px;"><small class="timestamp"><i class="fa fa-clock-o">'+"just now"+'</i></small></div></div><hr><div style="margin-left:10px; margin-top: -10px;" >'+comment+'</div></div>';
                document.getElementById("comment_list").innerHTML=y+x;
                comments=comments+1; 
                //alert(comments); 
               // document.getElementById("comment_count1").innerHTML=comments+" comments";
            }
    });
}
       ...
       console.log(data);
          ...
          document.getElementById("comment_list").innerHTML="";
           for(var i=0;i<obj.length;i++){
               var htmlstring='<div id="comments" class="row" style="border:1px solid #1ABC9C; width:548px;margin-left: 25px;margin-bottom:10px;" ><div style=" margin-top: 10px;padding-bottom: 8px;"><div style="float:left;margin-left: 10px;"><b>'+obj[i].commenter+'</b></div><div style="float:right; margin-top: 5px;margin-right:10px;"><small class="timestamp"><i class="fa fa-clock-o">'+obj[i].time_+'</i></small></div></div><hr><div style="margin-left:10px; margin-top: -10px;" >'+obj[i].comment+'</div></div>';
                var x=  document.getElementById("comment_list").innerHTML;
                document.getElementById("comment_list").innerHTML=x+htmlstring;
           }
       }
